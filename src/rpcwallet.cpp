...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Triangles address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current Triangles address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <Trianglesaddress> <account>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress <Trianglesaddress> <amount> [narration] [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <Trianglesaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <Trianglesaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <Trianglesaddress> [minconf=1]\n"
            "Returns the total amount received by <Trianglesaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <toTrianglesAddress> <amount> [minconf=1] [narration] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001" 
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH (const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Triangles address: ")+s.name_);
...
void ThreadTopUpKeyPool(void* parg)
{
    ...
    RenameThread("Triangles-key-top");
...
void ThreadCleanWalletPassphrase(void* parg)
{
    ...
    RenameThread("Triangles-lock-wa");
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <Trianglesaddress>\n"
            "Return information about <Trianglesaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <Trianglespubkey>\n"
            "Return information about <Trianglespubkey>.");
...
// Triangles: reserve balance from being staked for network protection
...
// Triangles: check wallet integrity
...
// Triangles: make a public-private key pair
...
Value getnewstealthaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Triangles stealth address for receiving payments anonymously.  ");
...
Value sendtostealthaddress(const Array &params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid Triangles stealth address."));
        return result;
    };
...
// Triangles: Verify blockchain authenticity
Value verifyblockchain(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            "verifyblockchain\n"
            "Verifies the authenticity of the Triangles blockchain.\n"
    ...
    if(blockchainStatus == -2)
        result = "The authenticity of the Triangles blockchain has not yet been verified.";
    else if(blockchainStatus == -1)
        result = "The Triangles blockchain is not fully sychronized.";
    else if(blockchainStatus == 0)
        result = "The Triangles blockchain synchronized, but it does not match the latest checkpoint hash at Block " + 
			boost::lexical_cast<std::string>(CWallet::LAST_REGISTERED_BLOCK_HEIGHT) + 
			" (which is registered and guaranteed by the Bitcoin blockchain). So you are most likely on a forked chain, please resync with official peers at https://triangles.org.";
    else
        result = "The Triangles blockchain is fully synchronized. It is authentic! It is guaranteed by the Bitcoin blockchain (the most secure immutable database in the world) up to Block " 
        		+ boost::lexical_cast<std::string>(CWallet::LAST_REGISTERED_BLOCK_HEIGHT) + ".";
