    ...
    while (fRepeat)
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CWalletTx)& item, mapWallet)
        {
            ...
            if (txdb.ReadTxIndex(wtx.GetHash(), txindex))
            {
                ...
                if (fUpdated)
                {
                    printf("ReacceptWalletTransactions found spent coin %s TRI %s\n", FormatMoney(wtx.GetCredit()).c_str(), wtx.GetHash().ToString().c_str());
                    wtx.MarkDirty();
                    wtx.WriteToDisk();
                }
            }
...
// Triangles: total coins staked (non-spendable until maturity)
...

    {
		...
		CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // Triangles: sub-cent change is moved to fee
...
// Triangles: check 'spent' consistency between wallet and txindex
// Triangles: fix wallet spent state according to txindex
...
// Triangles: disable transaction (only for coinstake)
